...
std::discrete_distribution<uint32_t>* res_dist;
std::discrete_distribution<uint32_t>* user_dist;

uint32_t gen_res(std::mt19937* gen)
{
  return (*res_dist)(*gen);
}

uint32_t gen_user(std::mt19937* gen)
{
  return (*user_dist)(*gen);
}

void gen_bin_txn(std::ofstream* f, std::mt19937* gen_r, std::mt19937* gen_u)
{
  auto key = gen_res(gen_r) + 1;
  auto user = gen_user(gen_u) + 1;
  /* printf("key is %u\n", key); */
  /* printf("user is %u\n", user); */
  int padding = 40 + 2 * 8;
...
/*   rand.init(ROW_COUNT, zipf_s, 1238); */
  ...
  res_dist = new std::discrete_distribution<uint32_t>(
    std::begin(NFT_RESRC_WEIGHTS), std::end(NFT_RESRC_WEIGHTS));
  user_dist = new std::discrete_distribution<uint32_t>(
    std::begin(NFT_USER_WEIGHTS), std::end(NFT_USER_WEIGHTS));
  ...
  //for (int i = 0; i < 1000; i++) 
    gen_bin_txn(&outLog, &gen_r, &gen_u);
