...
void gen_bin_txn(std::ofstream* f, std::mt19937* gen_s, std::mt19937* gen_r)
{
  auto sender = gen_sender(gen_s) + 1;
  uint32_t recver;
  do {
    recver = gen_recver(gen_r) + 1;
  } while (sender == recver);
...
/*   rand.init(ROW_COUNT, zipf_s, 1238); */
  ...
  sender_dist = new std::discrete_distribution<uint32_t>(
    std::begin(P2P_SENDER_WEIGHTS), std::end(P2P_SENDER_WEIGHTS));
  recver_dist = new std::discrete_distribution<uint32_t>(
    std::begin(P2P_RECVER_WEIGHTS), std::end(P2P_RECVER_WEIGHTS));
  ...
  //for (int i = 0; i < 1000; i++) 
    gen_bin_txn(&outLog, &gen_s, &gen_r);
